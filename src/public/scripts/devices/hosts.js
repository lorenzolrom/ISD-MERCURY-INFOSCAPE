...
function saveChanges(id)
{
    apiRequest('PUT', 'hosts/' + id, getFormData()).done(function(json){
        if(json.code === 204)
        {
            window.location.replace (baseURI + "hosts?NOTICE=Host updated");
        }
        else
        {
...
function createHost()
{
    apiRequest('POST', 'hosts', getFormData()).done(function(json){
        if(json.code === 201)
            window.location.replace(baseURI + 'hosts?NOTICE=Host created');
        else
        {
            showNotifications('error', json.data.errors);
            unveil();
...
function deleteHost(id)
{
    apiRequest('DELETE', 'hosts/' + id, {}).done(function(json){
        if(json.code === 204)
            window.location.replace(baseURI + 'hosts?NOTICE=Host deleted');
        else
        {
            showNotifications('error', json.data.errors);
            unveil();

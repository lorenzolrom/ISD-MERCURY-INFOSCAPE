...
function saveChanges(id)
{
    apiRequest('PUT', 'urlaliases/' + id, getForm()).done(function(json){
        if(json.code === 204)
            window.location.replace(baseURI + 'web/urlaliases?NOTICE=URL Alias updated');
        else
        {
...
function createEntry()
{
    apiRequest('POST', 'urlaliases', getForm()).done(function(json){
        if(json.code === 201)
            window.location.replace(baseURI + 'web/urlaliases?NOTICE=URL Alias created');
        else
        {
            showNotifications('error', json.data.errors);
            unveil();
...
function deleteEntry(id)
{
    apiRequest('DELETE', 'urlaliases/' + id, {}).done(function(json){
        if(json.code === 204)
            window.location.replace(baseURI + "web/urlaliases?NOTICE=URL Alias deleted");
        else
        {
            showNotifications('error', json.data.errors);
            unveil();
